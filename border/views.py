...
def index(request):
    borderTable = Border.objects.values('id', '제목', '작성자', '조회수','좋아요','싫어요').order_by('-id')
    print(borderTable)
...
def borderUpdate(request, border_id):
    if request.method=="POST":
        ...
        # os.mkdir(settings.MEDIA_ROOT+"\\"+str(border_id)+"\\")
        for x in request.FILES.getlist("files"):
            upLoadFile = open(settings.MEDIA_ROOT+"\\"+str(border_id)+"\\"+str(x), 'wb')
...
def borderDelete(request, border_id):
    ...
    # os.removedirs(settings.MEDIA_ROOT+"\\"+str(border_id)+"\\")
    shutil.rmtree(settings.MEDIA_ROOT+"\\"+str(border_id)+"\\")
...
def good(request, border_id):
    border = Border.objects.get(id=border_id)
    border.좋아요 += 1
    border.save()
    
    return redirect("BD:D", border_id)

def hate(request, border_id):
    border = Border.objects.get(id=border_id)
    border.싫어요 += 1
    border.save()
